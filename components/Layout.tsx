    ...
    <div className="flex flex-col min-h-screen relative font-sans">
        ...
        <div className="container mx-auto px-4 flex justify-between items-center">
          <div className="flex items-center gap-6 font-medium">
             <div className="flex items-center gap-2 hover:text-accent transition-colors cursor-pointer">
               <MapPin size={12} /> <span>{t.palestine}, {t.jerusalem}</span>
             </div>
             <div className="flex items-center gap-2 hover:text-accent transition-colors cursor-pointer">
               <Phone size={12} /> <span dir="ltr">+970 5XX XXX XXX</span>
             </div>
          </div>
            ...
            <div className="flex items-center gap-4 md:gap-6">
              <div className="hidden sm:flex relative items-center group">
                <input 
                  ...
                  className={`py-2.5 bg-white/5 border border-white/10 rounded-2xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-accent/40 focus:bg-white/10 w-32 md:w-64 transition-all duration-500 focus:w-80 placeholder:text-white/20 font-medium ${lang === 'en' ? 'pl-4 pr-12' : 'pr-4 pl-12'}`}
                />
                <div className={`absolute pointer-events-none transition-all duration-300 group-focus-within:text-accent text-white/30 ${lang === 'en' ? 'right-4' : 'left-4'}`}>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 bg-primary/40 backdrop-blur-sm md:hidden" onClick={() => setIsMobileMenuOpen(false)}>
          <div className={`absolute top-0 ${lang === 'en' ? 'left-0' : 'right-0'} w-4/5 h-full bg-primary p-8 shadow-2xl overflow-y-auto animate-fade-in`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-16">
             <div className="flex flex-col space-y-6">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-2xl font-black tracking-tighter text-white uppercase">LAVIVAA</span>
                </div>
                <p className="text-white/60 text-base leading-relaxed font-medium max-w-xs">{t.footerDesc}</p>
                <div className="flex gap-4">
             ...
             <div className="flex flex-col">
                <h4 className="font-black text-xl mb-8 text-accent uppercase tracking-wider">{t.contact}</h4>
                <div className="space-y-6">
                   <div className="flex items-center gap-4 group cursor-pointer">
                      ...
                      <div>
                         <span className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">{t.location}</span>
                         <span className="text-sm font-bold text-white/80 group-hover:text-white transition-colors">{t.palestine}</span>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center gap-4 group cursor-pointer">
                      ...
                      <div>
                         <span className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">{t.phone}</span>
                         <span className="text-sm font-bold text-white/80 group-hover:text-white transition-colors" dir="ltr">+970 5XX XXX XXX</span>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center gap-4 group cursor-pointer">
                      ...
                      <div>
                         <span className="block text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">{t.email}</span>
          ...
          <div className="border-t border-white/5 pt-10 flex flex-col md:flex-row justify-between items-center gap-6 text-xs font-bold text-white/30">
             ...
             <div className="flex gap-8">
                <span className="hover:text-white transition-colors cursor-pointer">{lang === 'en' ? 'Privacy Policy' : 'سياسة الخصوصية'}</span>
                <span className="hover:text-white transition-colors cursor-pointer">{lang === 'en' ? 'Terms of Service' : 'الشروط والأحكام'}</span>
             </div>
          </div>
        </div>
      </footer>
      ...
      {isCartOpen && (
        <>
          ...
          <div className={`fixed top-0 bottom-0 ${lang === 'en' ? 'right-0' : 'left-0'} w-full md:w-[450px] bg-white z-50 shadow-2xl flex flex-col transform transition-all duration-500`}>
             ...
             <div className="flex-1 overflow-y-auto p-6">
               {cart.length === 0 ? (
                 <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-6">
                   ...
                   <button onClick={() => setIsCartOpen(false)} className="bg-primary text-white px-8 py-3 rounded-2xl font-black shadow-lg">{t.shopNow}</button>
                 </div>
               ) : (
                 <div className="space-y-6">
                    {cart.map(item => (
                      <div key={item.id} className="flex gap-5 bg-slate-50 p-4 rounded-3xl border border-slate-100 group">
                        ...
                        <div className="flex-1 flex flex-col">
                          ...
                          <div className="flex justify-between items-end mt-auto">
                             ...
                             <button onClick={() => removeFromCart(item.id)} className="text-red-400 text-xs font-bold hover:text-red-600 transition-colors bg-white px-3 py-1 rounded-full border border-red-50">
             ...
             {cart.length > 0 && (
               <div className="p-8 bg-slate-900 text-white rounded-t-[3rem] shadow-2xl">
                  ...
                  <Link 
                    ...
                    className="w-full bg-accent text-primary py-5 rounded-2xl font-black hover:bg-white transition-all block text-center shadow-xl uppercase tracking-widest"
                  >
                    {t.checkout}
                  </Link>
               </div>
